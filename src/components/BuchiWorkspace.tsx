...
function regexToString(node: import("@shared/utils").RegexNode | null): string {
  ...
  const wrap = (child: string, childPrec: number) =>
    childPrec < prec ? `(${child})` : child;
...
export function BuchiWorkspace() {
  ...
  const exportFormalLatex = () => {
    ...
    triggerDownload(
      latex,
      `${autoName || "automaton"}-definition.tex`,
      "text/x-tex",
    );
  };
  ...
  const exportLatex = () => {
    ...
    const latex = buildTikzExport(parsed.automaton, nodes, edges, {
      scale: 0.06,
    });
  ...
  >(() => {
    ...
    if (!parsed.automaton) {
      return {
        kind: "warning",
        message: "Define an automaton before testing a word.",
      };
    }
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-300">
              Enter the DSL on the left; the graph updates on the right.
            </p>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  size="sm"
                  variant="outline"
                  className="border-cyan-500/40 text-slate-100"
                >
                  ...
                  >
                    ...
                    {sample.languageLatex ? (
                      <span className="text-[11px] text-slate-300">
                        <MathText expression={sample.languageLatex} />
                      </span>
                    ) : null}
                  </DropdownMenuItem>
                ))}
          ...
          <div className="space-y-3 lg:col-span-5">
            ...
            {parsed.automaton ? (
              <div className="grid grid-cols-2 gap-3 text-sm text-slate-200">
                <InfoPill
                  label="States"
                  value={parsed.automaton.states.join(", ")}
                />
                <InfoPill
                  label="Alphabet"
                  value={parsed.automaton.alphabet.join(", ")}
                />
                <InfoPill label="Start" value={parsed.automaton.initial} />
                <InfoPill
                  label="Accepting"
                  value={parsed.automaton.accepting.join(", ")}
                />
              </div>
            ) : null}
          </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={savedAutomata.length === 0}
                  >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="min-w-[220px] bg-slate-900 text-slate-50">
                  ...
                  <DropdownMenuItem
                    onSelect={exportFormalLatex}
                    disabled={!parsed.automaton}
                    className="gap-2"
                  >
      ...
      <section className="mt-8 space-y-4 rounded-2xl border border-slate-800/60 bg-slate-950/60 p-5">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Enter a regex that ends with <MathText expression="{}^{\omega}" />{" "}
            (supports <MathText expression="|" />, <MathText expression="*" />,{" "}
            <MathText expression="+" />, parentheses). Example:{" "}
            <MathText expression="ab(ba)^{\omega}" />.
          </p>
        </div>
...
function WordStatusBanner({
  ...
  return (
    <div
      className={`rounded-xl border px-3 py-3 text-sm shadow-lg shadow-black/10 backdrop-blur ${toneClass}`}
    >
      <div className="flex items-center gap-2 font-semibold">
        {icon}
        <span>{label}</span>
      ...
      {status.details ? (
        <div className="mt-3 grid gap-2 text-xs text-white/90 sm:grid-cols-2">
          <DetailRow
            label="Input"
            value={<MathText expression={status.details.inputLatex} />}
          />
          <DetailRow
            label="Prefix"
            value={<MathText expression={status.details.prefixLatex} />}
          />
          <DetailRow
            label="Loop"
            value={<MathText expression={status.details.loopLatex} />}
          />
          <DetailRow
            label="Cycle"
            value={
          ...
          <DetailRow
            ...
            label="Reason"
            value={
              <span className="text-white/80">{status.details.reason}</span>
            }
          />
        </div>
      ) : null}
