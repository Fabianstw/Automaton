...
export function buildTikzExport(
  ...
  const width = nodes.length > 0 ? (Math.max(...nodes.map((n) => n.x)) - Math.min(...nodes.map((n) => n.x))) * baseScale : 0
  ...
  if (initialNode) {
    lines.push(`  \\draw[->] (${(initialNode.x * scale - 1).toFixed(2)},${(-initialNode.y * scale).toFixed(2)}) -- (${initialNode.id});`)
  }
  ...
  edges.forEach((e) => {
    if (e.source === e.target) {
      ...
      lines.push(`  \\draw[->] (${e.source}) -- node[sloped,above]{${e.label}} (${e.target});`)
    }
  })
...
export function buildFormalDefinitionLatex(automaton: BuchiAutomaton): string {
  const transitionsByState = automaton.transitions.reduce<Record<string, typeof automaton.transitions>>((acc, t) => {
    if (!acc[t.from]) acc[t.from] = []
    acc[t.from].push(t)
    return acc
  }, {})

  const transitionLines = Object.entries(transitionsByState).map(([from, list], index, arr) => {
    const entries = list.map((t) => `${t.symbol} \\mapsto ${t.to}`).join("; \\; ")
  ...
  if (transitionLines.length > 0) {
    lines.push("", "% Transition function by origin state", "\\[ \\begin{array}{rcl}")
