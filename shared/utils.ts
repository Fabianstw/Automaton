...
export function parseOmegaWord(raw: string): OmegaParseResult {
  ...
  try {
    ...
    if (omegaCount === 0) {
      return {
        ok: false,
        error: "Add a ^w suffix to mark the infinite loop (e.g., ab(ba)^w).",
      };
    }
    ...
    if (!components) {
      return {
        ok: false,
        error: "Place the ^w suffix at the end so the word looks like αβ^w.",
      };
    }
    ...
    const message =
      error instanceof RegexParseError
        ? error.message
  ...
  switch (node.kind) {
    ...
    case "concat":
      return node.nodes.reduce(
        (sum, child) => sum + estimateRegexComplexity(child),
        0,
      );
    case "union":
      return node.nodes.reduce(
        (max, child) => Math.max(max, estimateRegexComplexity(child)),
        0,
      );
    case "star":
    case "plus":
...
class RegexParser {
  ...
  parse(): RegexNode {
    ...
    if (!this.isAtEnd()) {
      throw new RegexParseError(
        `Unexpected character "${this.peek()}" at position ${this.index}.`,
      );
    }
  ...
  private parsePrimary(): RegexNode {
    ...
    throw new RegexParseError(
      `Unexpected character "${char}" at position ${this.index}.`,
    );
  }
...
function extractOmegaComponents(
  ...
  if (ast.kind === "concat" && ast.nodes.length > 0) {
    ...
    return { prefix: prefix as RegexNode | null, omega: last.node };
  }
